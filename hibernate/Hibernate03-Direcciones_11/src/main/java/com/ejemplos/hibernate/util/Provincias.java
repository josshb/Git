 ...
 *
 * @author Codeko Diversas utilidades relacionadas con las provincias españolas
 */
public class Provincias {
     ...
     * @return String con el nombre de lap provincia a la que pertenece el
     * código postal indicado.
     */
    public static String getProvincia(String cp) {
        // Si tiene 5 cifras cojo los dos primeros.
        //Si tiene 4, solo cojo la primera y le pego un 0
        if (cp.length() == 5) {
            int cod = getInt((cp.trim() + "  ").substring(0, 2));
            return getProvincia(cod);
        } else {
            //Tiene 4 cifras
            int cod = getInt((cp.trim() + "  ").substring(0, 1));
            return getProvincia(cod);
    ...
    /**
     * Convierte objetos en enteros. Es capaz de convertir cualquier tipo de
     * Number a Integer y para el resto de objetos se usa
     * Integer.parseInt(obj.toString().trim())
     *
     * @param obj Objeto a convertir en entero
     * @param porDefecto Valor que tomará el entero si no se puede convertir el
     * objeto
     * @return La conversión a entero del objeto o el valor por defecto si no se
     * ha podido convertir
     */
