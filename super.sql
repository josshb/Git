...
CREATE INDEX `fk_producto_categorias1_idx` ON `mydb`.`producto` (`categorias_idcategorias` ASC) VISIBLE;
...
CREATE INDEX `ing_fk_platos` ON `restaurante`.`ingredientes` (`plato_id` ASC) VISIBLE;
...
CREATE INDEX `mp_fk_platos` ON `restaurante`.`menus_platos` (`plato_id` ASC) VISIBLE;

CREATE INDEX `mp_fk_menus` ON `restaurante`.`menus_platos` (`menu_id` ASC) VISIBLE;
